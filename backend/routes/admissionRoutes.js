...
admissionRouter.post("/complete", [
  ...
  body("academicInfo.previousEducation.percentage").isFloat({ min: 0, max: 100 }).withMessage("Percentage must be between 0 and 100").toFloat(),
  body("academicInfo.previousEducation.yearOfPassing").isInt({ min: 2000, max: new Date().getFullYear() }).withMessage("Valid year of passing is required").toInt(),
  // Family info is now optional - checkFalsy treats empty strings as optional
  body("familyInfo.father.name").optional({ checkFalsy: true }).trim().isLength({ min: 2 }).withMessage("Father's name must be at least 2 characters"),
  body("familyInfo.father.phone").optional({ checkFalsy: true }).matches(/^[6-9]\d{9}$/).withMessage("Valid Indian phone number is required"),
  body("familyInfo.father.occupation").optional({ checkFalsy: true }).trim().isLength({ min: 2 }).withMessage("Father's occupation must be at least 2 characters")
], completeApplication);
